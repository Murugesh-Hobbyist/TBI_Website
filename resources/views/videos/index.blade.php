...
@push('scripts')
    <script>
        ...
        function startProjectVideo(stage) {
            ...
            stage.innerHTML = '<div class="project-video-player h-full w-full"></div><button type="button" class="project-video-surface absolute inset-0 z-10 cursor-pointer" aria-label="Pause video"></button><div class="project-video-controls pointer-events-none absolute inset-x-0 bottom-0 z-20 h-[58px] bg-[#0B2441] opacity-0 transition-opacity duration-200 md:h-[64px]"><div class="flex h-full items-center gap-3 px-3 text-white md:px-4"><button type="button" class="project-video-restart pointer-events-auto inline-flex h-8 w-8 shrink-0 items-center justify-center rounded text-lg text-white/90" aria-label="Restart video">&#8634;</button><span class="project-video-time shrink-0 text-xs font-bold tabular-nums">0:00 / 0:00</span><input class="project-video-progress pointer-events-auto h-1.5 min-w-0 flex-1 cursor-pointer accent-[#FF0000]" type="range" min="0" max="0" value="0" step="0.1" aria-label="Video progress"><div class="group relative flex h-8 w-8 shrink-0 items-center justify-center"><div class="absolute bottom-full left-1/2 flex h-28 w-10 -translate-x-1/2 items-center justify-center opacity-0 transition-opacity group-hover:opacity-100"><input class="project-video-volume pointer-events-none h-24 w-1 cursor-pointer accent-[#FF0000] group-hover:pointer-events-auto" type="range" min="0" max="100" value="100" step="1" aria-label="Video volume" style="writing-mode: vertical-lr; direction: rtl;"></div><button type="button" class="project-video-mute pointer-events-auto inline-flex h-8 w-8 items-center justify-center rounded text-base text-white/90" aria-label="Mute video">&#128266;</button></div><button type="button" class="project-video-captions pointer-events-auto rounded px-2 py-1 text-xs font-extrabold text-white/80" aria-label="Turn captions on">CC</button><button type="button" class="project-video-fullscreen pointer-events-auto inline-flex h-8 w-8 shrink-0 items-center justify-center rounded text-xl text-white/90" aria-label="Enter fullscreen">&#9974;</button></div></div>';
            ...
            function holdControls() { window.clearTimeout(state.controlsTimer); state.controlsTimer = null; showControls(); }
            ...
            controls.addEventListener('mouseenter', holdControls);
            controls.addEventListener('mouseleave', showForSixSeconds);
