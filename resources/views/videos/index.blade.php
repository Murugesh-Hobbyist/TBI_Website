            ...
            <div class="mt-5 grid gap-4 md:grid-cols-3">
                @foreach ($videos as $video)
                    <a href="{{ route('videos.show', $video) }}" class="tb-card tb-reveal">
                        <div class="text-xs font-extrabold uppercase tracking-[0.12em] text-[#597696]">Video</div>
                        <h2 class="mt-2 font-display text-xl text-[#112743]">{{ $video->title }}</h2>
                        @if ($video->summary)
                            <p class="mt-2 text-sm leading-relaxed text-[#4C6686]">{{ \Illuminate\Support\Str::limit(strip_tags((string) $video->summary), 140) }}</p>
                        @endif
                        <div class="mt-4 text-sm font-bold text-[#0B6ECD]">Watch now</div>
                    </a>
                @endforeach

                @if (($dbOk ?? true) && $videos->count() === 0)
                    <div class="tb-panel p-6 text-sm text-[#4C6686] md:col-span-3 tb-reveal">No published videos yet.</div>
                @endif
            </div>
